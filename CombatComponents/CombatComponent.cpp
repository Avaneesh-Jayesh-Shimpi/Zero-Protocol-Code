...
#include "Character/BlockAnimInstance.h"
...
void UCombatComponent::ServerReload_Implementation()
{
	...
	if (!Character->IsLocallyControlled()) HandleReloadMontage();
}
...
void UCombatComponent::UpdateAmmoHUD()
{
	...
	if (Controller)
	{
		...
		// SAVE: Always sync local changes to the persistent Controller map
		if (Controller->StoredAmmoMap.Contains(EquippedWeapon->GetWeaponType()))
		{
			Controller->StoredAmmoMap[EquippedWeapon->GetWeaponType()] = CarriedAmmo;
		}
		else
		{
			Controller->StoredAmmoMap.Emplace(EquippedWeapon->GetWeaponType(), CarriedAmmo);
		}
	}
}
...
void UCombatComponent::UpdateAmmoValues()
{
	...
	// 2. Update HUD & SAVE to Controller
	UpdateAmmoHUD();	
...
void UCombatComponent::OnRep_CombatState()
{
	switch (CombatState)
	{
		case ECombatState::ECS_Reloading:
			if (Character && !Character->IsLocallyControlled()) HandleReloadMontage();
			break;
...
void UCombatComponent::InitializeCarriedAmmo()
{
	...
	// CHECK: If Controller has ammo saved, use it!
	if (Controller && !Controller->StoredAmmoMap.IsEmpty())
	{
		CarriedAmmoMap = Controller->StoredAmmoMap;
		
		// If we have an equipped weapon, make sure the HUD matches immediately
...
void UCombatComponent::ExecuteFireWeaponLogic()
{
	if (CanFire())
	{
		bCanFire = false;
...
void UCombatComponent::FireProjectileWeapon()
{
	if (EquippedWeapon && Character)
	{
		...
		ServerFireWeapon(HitTarget, EquippedWeapon->FireDelay);
	}
}
...
void UCombatComponent::ServerFireWeapon_Implementation(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	MulticastFire(TraceHitTarget);
}

bool UCombatComponent::ServerFireWeapon_Validate(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	if (EquippedWeapon)
	{
...
void UCombatComponent::SetCrosshairPackage(float DeltaTime)
{
	if (Controller)
	{
		HUD = HUD == nullptr ? Cast<ABlockHUD>(Controller->GetHUD()) : HUD;
		if (HUD)
		{
			if (EquippedWeapon)
			{
