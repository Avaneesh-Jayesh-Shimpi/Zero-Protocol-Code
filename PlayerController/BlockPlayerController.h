...
UCLASS()
class ZEROPROTOCOL_API ABlockPlayerController : public APlayerController
{
	GENERATED_BODY()
...
public:
	...
	TMap<EWeaponType, int32> StoredAmmoMap;
	...
	void UpdateDeathMessageHUD(const FString KilledBy);
...
protected:
	...
	//Report's the Current Server time to client in response to ServerRequestServerTime
	...
	FString GetInfoText(const TArray<class ABlockPlayerState*>& Players);
	FString GetTeamsInfoText(class ABlockGameState* BlockGameState);
private:
